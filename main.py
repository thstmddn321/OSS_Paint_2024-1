...
#단축키를 이용한 화면 확대 및 축소 기능을 위한 함수
def zoom_with_key(event):
    if event.state & 0x1:  # Shift 키가 눌렸는지 확인
        if event.keysym == "Up":
            scale = 1.1
        elif event.keysym == "Down":
            scale = 0.9  # "+" 키는 확대, "-" 키는 축소
        else:
            return
        canvas.scale("all",event.x, event.y,scale, scale)
    else:
...
window.bind("<Shift-Up>", zoom_with_key)  # Shift + 위쪽 화살표 키로 확대
window.bind("<Shift-Down>", zoom_with_key)  # Shift + 아래쪽 화살표 키로 축소
